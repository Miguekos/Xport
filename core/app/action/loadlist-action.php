...
//Asistencia
    ...
    if ($idd !== null) {
        ...
        while($row = mysqli_fetch_array($result)) {
            $namee = $row['nombre'];
            $iddr = $row['id'];
            echo " <a class='btn btn-primary' href='asistencia.php?q=$iddr'>$namee</a><br>";
            echo "<br>";
            ...
            //echo $ffin;
             echo "<th>Mostrando los Ultimos 15 dias de asistencia.. Para ver el registro completo Precione en su nombre..!!</th>";
			
		//$sql="SELECT * FROM `assistance` WHERE `person_id` = '".$q."'";
        $sql="SELECT * FROM (SELECT * FROM `assistance` WHERE `person_id` = '".$q."' ORDER BY ID DESC LIMIT 15) sub ORDER BY `ID` DESC";
