...
$nombre = "Deudores_".$fechaA.".xlsx";

 $conexion = new mysqli($server,$name_db,$pass_db,$db);
 $sql = "SELECT * FROM cliente WHERE deuda > 0";
 ...
 if ($registros > 0) {
   ...
   $objPHPExcel = $objReader->load("Deudores.xlsx");
   ...
   while ($registro = mysqli_fetch_object($resultado)) {
      ...
      $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E'.$i, $registro->dni)
            ->setCellValue('F'.$i, $registro->monto)
            ->setCellValue('G'.$i, $registro->deuda)
